...
//!
//! Raw expressions from the collector enter through `resolve_typeof_expression`.
//! Expressions that are already stored in the database enter through
//! `resolve_inferred_typeof_expression`; the remaining helpers handle calls,
//! operators, conditionals, and member access for both forms.
...
impl<'db> ResolutionCtx<'db, '_> {
    /// Resolves a deferred expression from the collector's raw type table.
    ...
    /// Resolves a deferred expression whose child types are already inferred.
    ...
    fn resolve_await_expression(
        ...
        let mut remaining_steps = MAX_AWAIT_EXPRESSION_STEPS;
    ...
    /// Finds the type of a named property on an inferred object type.
    ...
    fn conditional_type(&mut self, ty: InferredTypeData<'db>) -> ConditionalType {
        ...
        let mut seen = FxHashSet::default();
        let mut pending = Vec::from([ty]);
        let mut remaining_steps = MAX_CONDITIONAL_TYPE_STEPS;
    ...
    fn filter_type_to_subset(
        ...
        let mut seen = FxHashSet::default();
        let mut pending = Vec::from([ty]);
        let mut remaining_steps = MAX_CONDITIONAL_FILTER_STEPS;
        ...
        while let Some(ty) = pending.pop() {
            ...
            if remaining_steps == 0 {
                return ConditionalFilterResult::Indeterminate;
            }
            remaining_steps -= 1;
        ...
        ConditionalFilterResult::Complete(collected_type_result(self.db, types))
    }
