...
#[test]
fn test_named_export_resolution_does_not_charge_duplicate_reexports() {
    const LIMIT: usize = 1024;

    let fs = MemoryFileSystem::default();
    fs.insert("/src/leaf.ts".into(), "export const value = 1;");
    fs.insert(
        "/src/barrel.ts".into(),
        std::iter::repeat_n("export * from './leaf';", LIMIT + 1).collect::<String>(),
    );
    fs.insert(
        "/src/index.ts".into(),
        "import { value } from './barrel'; export { value };",
    );
    let db = build_js_test_module_db(
        &fs,
        &["/src/leaf.ts", "/src/barrel.ts", "/src/index.ts"],
        true,
    );
    let module = db
        .module_for_path(Utf8Path::new("/src/index.ts"))
        .expect("importer module must exist");
    let inferred = infer_module_types_bottom_up(&db, module).expect("types must be inferred");
    let value_ty = inferred_binding_ty_by_name(&db, module, &inferred, "value")
        .expect("imported value must exist");

    assert!(contains_inferred_number_literal(&db, value_ty, "1"));
}
